...
int main()
{
	...
	{
		list<size_t> firstList;
		list<size_t> secondList;

		for (int i = 0; i < forty_thousand; ++i)
		{
			firstList.push_back(randomInt(engine)); // populate the first list with random number
		}

		for (int i = 0; i < forty_thousand; ++i)
		{
			secondList.push_back(randomInt(engine)); // populate the second list with random number
			firstList.merge(secondList); // merge second list to the first one.
		}
	}
...

	{
		...
		for (int i = 0; i < forty_million; ++i) 
		{
			a1[i] = randomInt(engine);// random number 1 to 4 billion
		}
		sort(a1, a1 + forty_million);
		delete[] a1;
	}
	...
	{
		multiset<size_t,less<size_t>> m1;
		for (int i = 0; i < four_million; ++i)
		{
			m1.insert(randomInt(engine));
		}
	}
...

	{
		...
		for (int i = 0; i < four_million; ++i)
		{
			l1.push_back(randomInt(engine)); // populate list with random number
		}
		l1.sort();
		l1.unique();
	}
	...
	{
		set<size_t, less<size_t>> s1;
		while (s1.size() != four_million) 
		{
			s1.insert(randomInt(engine));
		}
	}
